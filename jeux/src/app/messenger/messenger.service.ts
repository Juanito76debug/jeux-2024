...
export class MessengerService {
    ...
    private apiUrl = "http://localhost:4200/api/messenger";
    ...
    openDiscussion(subject: string): Observable<Discussion> {
        ...
        return this.http.post<Discussion>(`${this.apiUrl}/discussions`, newDiscussion);
    }
    ...
    sendMessage(discussionId: string, content: string): Observable<Message> {
        ...
        const updatedDiscussions = this.discussions.value.map(discussion => {
            if (discussion.id === discussionId) {
              return { ...discussion, messages: [...discussion.messages, newMessage] };
            }
            return discussion;
          });
            ...
            return this.http.post<Message>(`${this.apiUrl}/discussion/${discussionId}/messages`, newMessage);
        }
        ...
        deleteDiscussion(discussionId: string): Observable<any> {
            this.discussions.next(this.discussions.value.filter(discussion => discussion.id !== discussionId));
...
deleteMessage(discussionId: string, messageId: string): Observable<any> {
    const updatedDiscussions = this.discussions.value.map(discussion => {
      if (discussion.id === discussionId) {
        return { ...discussion, messages: discussion.messages.filter(message => message.id !== messageId) };
      }
      return discussion;
    });;

        this.discussions.next(updatedDiscussions);
        return this.http.delete(`${this.apiUrl}/discussion/${discussionId}/messages/${messageId}`);
